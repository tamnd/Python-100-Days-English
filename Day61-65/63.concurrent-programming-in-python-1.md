...
A so-called daemon thread is an execution thread that is not worth keeping after the main thread ends. The meaning of "not worth keeping" here is that daemon threads are destroyed after all other non-daemon threads finish running. What they guard are all non-daemon threads inside the current process. Simply speaking, daemon threads die along with the main thread, and the life cycle of the main thread is the life cycle of a process. If this is not clear, we can look at a simple piece of code.
