...
In traditional Web application development, most programmers treat the browser as the dividing line between frontend and backend. The part that shows pages to users in the browser is called the frontend, and all the code running on the server to provide business logic and prepare data for the frontend is called the backend. Frontend-backend separation means frontend engineers and backend engineers agree on the data interaction APIs, then develop and test in parallel. The backend only provides data and does not render the data into pages. The frontend gets data through HTTP requests and renders the data into pages. This work is done by JavaScript code in the browser.
...
In the code above, we loop through the `QuerySet` object obtained by querying subjects, turn each subject's data into a dictionary, save the dictionaries in a list container named `subjects`, and finally use `JsonResponse` to serialize the list and return JSON data to the browser. Because `JsonResponse` is serializing a list instead of a dictionary, we need to set the value of the `safe` parameter to `False` so it can serialize `subjects`; otherwise, a `TypeError` exception will happen.
...
Then access this API, and you can get JSON data like the following.
...
For the detailed usage guide of `bpmappers`, please refer to its [official documentation](https://bpmappers.readthedocs.io/en/stable/). This official documentation is written in Japanese, and you can use the browser's translation function to translate it into the language you know best.
