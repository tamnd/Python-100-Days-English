...
- The `collections` module
  ...
  - `Counter`: a subclass of `dict`. The key is the element, and the value is the count of the element. Its `most_common()` method can help us get the elements with the highest frequency. I think the inheritance relationship between `Counter` and `dict` is worth discussing. According to the CARP principle, the relationship between `Counter` and `dict` should be designed as an association relationship, which is more reasonable.
...
- Common algorithmic strategies:
  ...
  ```python
  """
  Quicksort: choose a pivot to partition the elements.
  The left side is all smaller than the pivot, and the right side is all larger than the pivot.
  """
  ...
  Example of backtracking: [Knight's tour](https://en.wikipedia.org/wiki/Knight%27s_tour).

  ```python
  """
  Recursive backtracking: also called trial method.
  Search forward according to the selection condition.
  When searching to some step, if the original choice is not good enough
  or cannot reach the goal, then step back and choose again.
  Classic problems include knight's tour, eight queens, and maze path finding.
  """
  import sys
  import time
  ...
  > **Note**: A sublist means a list made up of elements whose indexes are continuous in the list. The elements in the list are of type `int`, and may include positive integers, `0`, and negative integers. The program inputs the elements in the list, and outputs the maximum value of the sum of elements in a sublist. For example:
  >
  > Input: `1 -2 3 5 -3 2`
  >
  > Output: `8`
  ...
  > **Note**: The easiest solution to think of for this problem is using a two-level loop, but the time performance of the code will become very bad. Using the idea of dynamic programming, only two more variables are used, and the original problem with `O(N^2)` complexity becomes `O(N)`.
...
- Metadata of parameters, issue of code readability

- Usage of anonymous functions and inline functions, `lambda` functions

- Closures and the issue of scope

  - The LEGB order in which Python searches for variables, `Local >>> Embedded >>> Global >>> Built-in`

  - The functions of the `global` and `nonlocal` keywords
    ...
    `nonlocal`: declare using a variable in the nested scope. The variable must exist in the nested scope, otherwise an error is reported.
